...
// We start search the matrix from top right corner, initialize the 
// current position to top right corner, if the target is greater than 
// the value in current position, then the target can not be in entire
// row of current position because the row is sorted, 
// if the target is less than the value in current position, then the 
// target can not in the entire column because the column is sorted too.
